-- Local fork of xmake-repo's cef package.
--
-- Upstream pins the wrapper to the static CRT (vs_runtime = "MT", readonly), which
-- can't be overridden via add_requireconfs. Everything else in this project builds
-- against the dynamic CRT (/MD), so linking an executable against both produced
-- LNK2038 CRT mismatches and made the launcher targets unbuildable.
--
-- cef_dll_wrapper is compiled from source here (see port/xmake.lua), and it only
-- talks to the prebuilt libcef.dll across CEF's C API boundary - no CRT resources
-- cross that boundary - so it is safe to build it with whichever CRT the consuming
-- application uses. Dropping the pin lets it inherit the project's runtime, which
-- is what upstream CEF's own CMake exposes via CEF_RUNTIME_LIBRARY_FLAG.
--
-- Only difference from upstream: the add_configs("vs_runtime", ...) line is gone.
package("cef")
