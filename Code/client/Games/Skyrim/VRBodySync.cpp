...
void CollectBones(void* apObject, BoneNodes& aNodes, uint32_t& aFound, uint32_t aDepth) noexcept
{
    if (!apObject || aDepth > 64 || aFound == VRPose::kBoneCount)
        return;

    if (const char* pName = GetName(apObject))
    {
        for (uint32_t i = 0; i < VRPose::kBoneCount; ++i)
        {
            if (!aNodes[i] && _stricmp(pName, kBoneNames[i]) == 0)
            {
                aNodes[i] = apObject;
                ++aFound;
                break;
    ...
    for (uint16_t i = 0; i < capacity; ++i)
        CollectBones(pChildren[i], aNodes, aFound, aDepth + 1);
}
...
bool FindBones(void* apRoot, BoneNodes& aNodes) noexcept
{
    aNodes.fill(nullptr);
    uint32_t found = 0;
    CollectBones(apRoot, aNodes, found, 0);
    return found == VRPose::kBoneCount;
}
...
void ApplyRemotePose(Actor* apActor) noexcept
{
    ...
    // Propagate to everything under the spine (fingers, head parts, held weapons, armour nodes).
    UpdateSubtree(nodes[VRPose::kSpine1], 0);
}

} // namespace

// Actor::UpdateAnimation (SE 36370, VR 0x5e1f10 - identified in live code as the vtable slot 0x7D
// target). The engine calls it directly from the process update rather than through the vtable, so
// it has to be detoured; the pose is applied right after the game animated the actor.
...
static TiltedPhoques::Initializer s_vrBodySyncHooks(
    []()
    {
        POINTER_SKYRIMSE(TUpdateAnimation, s_updateAnimation, 0, 36370);
