# Skyrim Together VR

Co-op for Skyrim VR, a port of Skyrim Together Reborn.

Pick the file for your situation:

- **`README-mod-manager.md`**: you use Mod Organizer 2 or a Wabbajack list (FUS, and the like).
- **`README-manual.md`**: no mod manager, or Vortex.
- **`README-host.md`**: you are the one running the server.

Everyone needs Skyrim VR 1.4.15, SKSE VR and the VR Address Library for SKSEVR, and the same build as
the others. Problems: run `collect-logs.bat` in the launcher folder and post the zip it puts on your Desktop.

Skyrim Together Reborn is by Tilted Phoques and is GPLv3; this port inherits that licence. VR address
research from the TiltedEvolutionVR project. The source of the build you have is available on request.
Neither upstream project is responsible for this port.
